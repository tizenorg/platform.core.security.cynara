...
Requires(pre): cynara-migrate >= %{version}-%{release}
Requires(post):   smack
...
if [ $1 -gt 1 ] ; then
    %{_sbindir}/cynara-migration.sh upgrade
else
    %{_sbindir}/cynara-migration.sh install
fi
...
%postun
if [ $1 = 0 ]; then
    %{_sbindir}/cynara-migration.sh uninstall
...
%attr(744,root,root) %{_sbindir}/cynara-migration.sh
