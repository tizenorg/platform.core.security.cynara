...
AdminPolicyParser::AdminPolicyParser(std::shared_ptr<std::istream> input) : m_input(input) {}

AdminPolicyParser::~AdminPolicyParser() {}

CynaraAdminPolicies AdminPolicyParser::parse() {
    CynaraAdminPolicies policies;

    for (std::size_t lineNum = 1; !m_input->eof(); ++lineNum) {
        std::string line;
        std::getline(*m_input, line, StorageSerializer::recordSeparator());
        ...
        try {
            ...
            policies.add(bucketId, policyType, metadata, policyKey);
