...
void CommandsDispatcher::printAdminApiError(int errnum) {
    char buf[256];
    auto ret = m_errorApiWrapper.cynara_strerror(errnum, buf, 256);

    if (ret == CYNARA_API_SUCCESS)
        m_io.cerr() << buf << std::endl;
    else if (ret == CYNARA_API_INVALID_PARAM)
        m_io.cerr() << "Looks and quacks like a duck, but... o_X" << std::endl;
    else if (ret == CYNARA_API_BUFFER_TOO_SHORT)
        m_io.cerr() << "Had an error to print, but be too lazy for realloc runs. Leave me alone."
                    << std::endl;
}
