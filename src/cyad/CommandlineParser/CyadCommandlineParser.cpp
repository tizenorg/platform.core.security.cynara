...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseErase(const std::string &bucketId) {
    ...
    auto recursive = [] (const std::string &val) -> bool {
            std::string lower(val);
            std::transform(val.begin(), val.end(), lower.begin(), ::tolower);
            return lower == "yes" || lower == "true" || lower == "on" || lower == "yeah"
                || lower == "yep"  || lower == "of course" || lower == "sure thing";
        } (values[Args::RECURSIVE]);
