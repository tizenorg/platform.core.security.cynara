...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseMain(void) {
    ...
    while ((opt = getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr)) != -1) {
        switch (opt) {
            ...
            case '?': // Unknown option
                return std::make_shared<ErrorCyadCommand>(Err::unknownOption());

            case ':': // Missing argument
                switch (optopt) {
                    case CmdlineOpt::SetBucket:
                    case CmdlineOpt::DeleteBucket:
                        return std::make_shared<ErrorCyadCommand>(Err::noBucket());
                }
                // Shall never happen, but let's just make compiler happy.
                return std::make_shared<ErrorCyadCommand>(Err::unknownError());
    ...
    return std::make_shared<ErrorCyadCommand>(Err::noOption());
}
...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseSetBucket(const std::string &bucketId) {
    ...
    while ((opt = getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr)) != -1) {
        ...
        case CmdlineOpt::Metadata:
            ...
            break;
        default:
            return std::make_shared<ErrorCyadCommand>(Err::unknownOption(CmdlineOpt::SetBucket));
        }
    }

    if (policy.empty())
        return std::make_shared<ErrorCyadCommand>(Err::noType());
    ...
    try {
        ...
        return std::make_shared<ErrorCyadCommand>(Err::invalidType());
    }
}
...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseDeleteBucket(const std::string &bucketId) {
    ...
    if (getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr) == -1) {
        ...
        return std::make_shared<ErrorCyadCommand>(Err::unknownOption(CmdlineOpt::DeleteBucket));
    }
}
...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseSetPolicy(void) {
    ...
    while ((opt = getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr)) != -1) {
        ...
        case ':': // Missing argument
            return std::make_shared<ErrorCyadCommand>(Err::argumentMissing(CmdlineOpt::SetPolicy));
        default:
            return std::make_shared<ErrorCyadCommand>(Err::unknownOption(CmdlineOpt::SetPolicy));
        }
    }
    ...
    for (const auto &val : values) {
        if (val.second.empty()) {
            ...
            return std::make_shared<ErrorCyadCommand>(Err::optionMissing(CmdlineOpt::SetPolicy));
        }
    }
...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseErase(const std::string &bucketId) {
    ...
    while ((opt = getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr)) != -1) {
        ...
        case CmdlineOpt::Privilege:
            ...
            break;
        case ':': // Missing argument
            return std::make_shared<ErrorCyadCommand>(Err::argumentMissing(CmdlineOpt::Erase));
        default:
            return std::make_shared<ErrorCyadCommand>(Err::unknownOption(CmdlineOpt::Erase));
        }
    }
    ...
    for (const auto &val : values) {
        if (val.second.empty()) {
            // TODO: Identify actual option
            return std::make_shared<ErrorCyadCommand>(Err::optionMissing(CmdlineOpt::Erase));
        }
    }
...
std::shared_ptr<CyadCommand> CyadCommandlineParser::parseCheck(void) {
    ...
    while ((opt = getopt_long(m_argc, m_argv, shortOpts.data(), longOpts.data(), nullptr)) != -1) {
        ...
        case CmdlineOpt::Bucket:
            ...
            break;
        case ':': // Missing argument
            return std::make_shared<ErrorCyadCommand>(Err::argumentMissing(CmdlineOpt::Check));
        default:
            return std::make_shared<ErrorCyadCommand>(Err::unknownOption(CmdlineOpt::Check));
        }
    }
    ...
    for (const auto &val : values) {
        if (val.second.empty()) {
            // TODO: Identify actual option
            return std::make_shared<ErrorCyadCommand>(Err::optionMissing(CmdlineOpt::Check));
        }
    }
