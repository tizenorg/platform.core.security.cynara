...
{
    checkId = 0;
    for (auto it = m_checkSet.begin(); it != m_checkSet.end(); ++it) {
        if (*it != checkId) {
            m_checkSet.insert(it, checkId);
            return true;
        }
        ++checkId;
    }
    if (checkId == static_cast<uint16_t>(-1))
        return false;
    m_checkSet.insert(m_checkSet.end(), checkId);
    return true;
}

bool CheckContainer::remove(cynara_check_id checkId)
{
    return m_checkSet.erase(checkId) == 1;
}

void CheckContainer::clear()
{
    return m_checkSet.clear();
}
