 ...
 */
/**
 * \file        socket-credentials-helper.h
...
#define SOCKET_CREDENTIALS_API_OPERATION_NOT_ALLOWED -5
 ...
 * \par Description:
 * Provides the caller with the user_id of a procces owner, the caller communicates with via a socket.
 *
 ...
 *
 * \param[in] connection a UNIX socket descriptor
 ...
 *
 */
int socket_credentials_get_user_id(const int connection, const char *method, void *user_id, unsigned int *length);
 ...
 * \param[in\out] the length of the buffer needed to store client_id, passing 0 causes returning this information
 *
 */
int socket_credentials_get_client_id(const int connection, const char *method, void *client_id, unsigned int *length);
