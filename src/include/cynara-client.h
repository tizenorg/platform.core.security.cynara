 ...
 *
 * \par Purpose:
 * This API should be used for a quick check if a user running application identified ad client
 ...
 * being one of CYNARA_API_PERMISSION_DENIED or CYNARA_API_PERMISSION_ALLOWED, no extensions are
 * run. Instead, CYNARA_API_ACCESS_NOT_RESOLVED is returned, meaning, that only running full
 * cynara_check() API would yield eventual answer.
 ...
 *
 * \par Important notes:
 * The answer will be taken from Cynara's database withour tunning any 3rd party extensions.
 * If the answer cannot be resolved int one of CYNARA_API_PERMISSION_DENIED or 
 * CYNARA_API_PERMISSION_ALLOWED without running extensions the API will return
