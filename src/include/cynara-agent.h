 ...
 *
 * \return CYNARA_AGENT_API_SUCCESS on success, or error code on error.
 */
 ...
 *
 * \par Purpose:
 * This API should be used to clean up after usage of cynara-agent library. Also it can be used to
 * stop execution of cynara_agent_get_request() and cynara_agent_put_response().
 *
 * \par Typical use case:
 ...
 * then returns with CYNARA_AGENT_API_SUCCESS code. Request type, request id and specific
 ...
 * return. If agent wants this function to return he must call cynara_agent_finish() function which
 * will close connection to cynara and from this moment only cynara_agent_initialize() may be
 * called. On success, buffer for plugin specific data is allocated and size is set. Developer is
 ...
 *                  CYNARA_AGENT_API_OUT_OF_MEMORY is returned and all arguments remain untouched.
 * \param[out] data_size Size of plugin data (bytes count). In case of out of memory this value
 *                  stays untouched.
 *
 * \return CYNARA_AGENT_API_SUCCESS on successfully read request, or negative error code otherwise.
 */
 ...
 * It is acceptable that agent sends responses of both types for one request.
 ...
 * \param[in] data Plugin specific data. Buffer is filled with data for corresponding plugin.
