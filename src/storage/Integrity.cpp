...
void Integrity::createBackupGuard(void) const {
    syncToStorage(m_dbPath + m_guardFilename, O_CREAT | O_EXCL | O_WRONLY | O_TRUNC);
}
...
void Integrity::syncElement(const std::string &filename, int flags, mode_t mode) {
    int fileFd = open(filename.c_str(), flags, mode);
...
void Integrity::syncToStorage(const std::string &filename, int flags, mode_t mode) const {
    // from: man 2 fsync
    ...
    syncElement(filename, flags, mode);
    syncElement(m_dbPath, O_DIRECTORY, mode);
}
...
void Integrity::createPrimaryHardLinks(const Buckets &buckets) {
    for (const auto &bucketIter : buckets) {
        ...
        syncToStorage(bucketFilename + m_backupFilenameSuffix);
    ...
    syncToStorage(indexFilename + m_backupFilenameSuffix);
