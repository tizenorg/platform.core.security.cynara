...
void ChecksumStream::save() {
    m_outStream->close();
    *m_chsStream << m_filename << PathConfig::StoragePath::bucketFieldSeparator
                 << generate(m_bufStream.str()) << PathConfig::StoragePath::bucketRecordSeparator;
}

const std::string ChecksumStream::generate(const std::string &data) const {
