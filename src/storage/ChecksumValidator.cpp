...
const std::string ChecksumValidator::generate(const std::string &data) {
    std::unique_ptr<char> checksum(crypt(data.c_str(), "$1$"));

    if (nullptr != checksum) {
        return std::string(checksum.release());
    } else {
        int err = errno;
        if (err == ENOSYS) {
...
void ChecksumValidator::compare(const std::shared_ptr<std::istream> &stream,
    ...
    std::unique_ptr<char> pathnameDuplicate(::strdup(pathname.c_str()));
    ...
    std::unique_ptr<char> basename(::basename(pathnameDuplicate.get()));
    std::string filename(basename.release());
