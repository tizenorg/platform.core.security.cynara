...
void ChecksumValidator::load(const std::shared_ptr<std::ifstream> &stream) {
    ...
    for (std::size_t lineNum = 1; !stream->eof(); ++lineNum) {
        std::string line;
        std::getline(*stream, line, PathConfig::StoragePath::bucketRecordSeparator);
...
void ChecksumValidator::compare(const std::shared_ptr<std::ifstream> &stream,
    ...
    if (m_sums[index] != generate(copyStream.str())) {
        throw ChecksumRecordCorruptedException(m_sums[index]);
    }
};

const std::string ChecksumValidator::parseFilename(const std::string &line,
                                                   std::size_t &beginToken) {
    auto filenameEndToken = line.find(PathConfig::StoragePath::bucketFieldSeparator, beginToken);
