...
void ProtocolAdmin::execute(RequestContextPtr context, EraseRequestPtr request) {
    LOGD("Serializing EraseRequest: sequenceNumber [%" PRIu16 "], startBucket <%s>, "
         ...
         request->sequenceNumber(), request->startBucket().c_str(), request->recursive(),
         request->filter().client().value().c_str(), request->filter().user().value().c_str(),
         request->filter().privilege().value().c_str());
