...
void ProtocolAdmin::execute(RequestContextPtr context, SetPoliciesRequestPtr request) {
    LOGD("Serializing SetPoliciesRequestPtr: sequenceNumber [%u], insertOrUpdate count [%u]",
         "remove count [%u]", static_cast<unsigned int>(request->sequenceNumber()),
         static_cast<unsigned int>(request->policiesToBeInsertedOrUpdated().size()),
         static_cast<unsigned int>(request->policiesToBeRemoved().size()));
