...
RequestPtr ProtocolAdmin::deserializeInsertOrUpdateBucketRequest(ProtocolFrameHeader &frame) {
    ...
    LOGD("Deserialized InsertOrUpdateBucketRequest: bucketId <%s>, "
            "result.type [%d], result.meta <%s>", policyBucketId.c_str(), (int )policyType,
            policyMetaData.c_str());
...
RequestPtr ProtocolAdmin::deserializeSetPoliciesRequest(ProtocolFrameHeader &frame) {
    ...
    for (int b = 0; b < toBeInsertedOrUpdatedCount; ++b) {
        PolicyBucketId policyBucketId;
        ProtocolDeserialization::deserialize(frame, policyBucketId);
    ...
    for (int b = 0; b < toBeRemovedCount; ++b) {
        PolicyBucketId policyBucketId;
        ProtocolDeserialization::deserialize(frame, policyBucketId);
        ProtocolDeserialization::deserialize(frame, policyCount);
        for (int p = 0; p < policyCount; ++p) {
    ...
    LOGD("Deserialized SetPoliciesRequestPtr: insertOrUpdate count [%d], remove count [%d]",
         toBeInsertedOrUpdatedCount, toBeRemovedCount);
...
ResponsePtr ProtocolAdmin::deserializeCodeResponse(ProtocolFrameHeader &frame) {
    ...
    // TODO: Bellow casting maniana ;) is to be changed but not now.
    // It involves to many changes. Will be fixed in future commit.
    uint16_t intcode;
    ProtocolDeserialization::deserialize(frame, intcode);
    code = (CodeResponse::Code)intcode;

    LOGD("Deserialized CodeResponse: code [%d], ", intcode);

    return std::make_shared<CodeResponse>(code, frame.sequenceNumber());
}
...
void ProtocolAdmin::execute(RequestContextPtr context, InsertOrUpdateBucketRequestPtr request) {
    LOGD("Serializing InsertOrUpdateBucketRequest: sequenceNumber [%d], bucketId <%s>, "
            "result.type [%d], result.meta <%s>", static_cast<int>(request->sequenceNumber()),
            request->bucketId().c_str(), (int )request->result().policyType(),
...
void ProtocolAdmin::execute(RequestContextPtr context, RemoveBucketRequestPtr request) {
    LOGD("Serializing RemoveBucketRequest: sequenceNumber [%d], bucketId <%s>",
            static_cast<int>(request->sequenceNumber()), request->bucketId().c_str());
...
void ProtocolAdmin::execute(RequestContextPtr context, SetPoliciesRequestPtr request) {
    LOGD("Serializing SetPoliciesRequestPtr: sequenceNumber [%d], insertOrUpdate count [%d]",
            "remove count [%d]", static_cast<int>(request->sequenceNumber()),
    ...
    ProtocolSerialization::serialize(*frame,
            (ProtocolFrameFieldsCount) request->policiesToBeInsertedOrUpdated().size());
    for (auto policyBucket : request->policiesToBeInsertedOrUpdated()) {
        ProtocolSerialization::serialize(*frame, policyBucket.first);
        ProtocolSerialization::serialize(*frame,
                    (ProtocolFrameFieldsCount) policyBucket.second.size());
    ...
    ProtocolSerialization::serialize(*frame,
            (ProtocolFrameFieldsCount) request->policiesToBeRemoved().size());
...
void ProtocolAdmin::execute(RequestContextPtr context, CodeResponsePtr response) {
    LOGD("Serializing CodeResponse: sequenceNumber [%d], code [%d]",
            static_cast<int>(response->sequenceNumber()), static_cast<int>(response->m_code));

    ProtocolFramePtr frame = ProtocolFrameSerializer::startSerialization(
            response->sequenceNumber());

    ProtocolSerialization::serialize(*frame, OpCodeRsponse);
    ProtocolSerialization::serialize(*frame, response->m_code);
