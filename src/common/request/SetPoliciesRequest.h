...
public:
    ...
    const std::map<PolicyBucketId, std::vector<Policy>> &insertOrUpdatePolicies(void) const {
        return m_insertOrUpdatePolicies;
    }

    const std::map<PolicyBucketId, std::vector<PolicyKey>> &removePolicies(void) const {
