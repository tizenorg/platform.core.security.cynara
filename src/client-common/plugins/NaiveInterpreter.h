...
class NaiveInterpreter : public InterpreterInterface {
    bool isUsable(const ClientSession &session UNUSED, const PolicyResult &result UNUSED) noexcept {
        return true;
    }
    bool isCacheable(const ClientSession &session UNUSED,
                     const PolicyResult &result UNUSED) noexcept
    {
        return true;
    }
    int toResult(const ClientSession &session UNUSED, const PolicyResult &result) noexcept {
