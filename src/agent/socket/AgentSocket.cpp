...
AgentSocket::AgentSocket(const std::string &socketPath, ProtocolPtr protocol)
        : m_socket(socketPath), m_protocol(protocol) {
}

ResponsePtr AgentSocket::askCynaraServer(RequestPtr request) {
...
RequestPtr AgentSocket::receiveRequestFromServer(void) {
    while (true) {
        if (!m_socket.waitAndReceiveFromServer(m_readQueue)) {
...
bool AgentSocket::sendDataToServer(BinaryQueue &data) {
    if (!m_socket.sendToServer(data)) {
...
bool AgentSocket::isConnected(void) {
    return m_socket.isConnected() && m_socket.receiveFromServer(m_readQueue);
}

AgentSocketState AgentSocket::connect(void) {
    if (isConnected())
