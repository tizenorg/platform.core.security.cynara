...
int Logic::ensureConnection(void) {
    ...
    if (ret == SS_DISCONNECTED) {
        LOGE("Agent socket disconnected.");
        return CYNARA_API_SERVICE_NOT_AVAILABLE;
    }

    if (ret == SS_RECONNECTED) {
...
int Logic::putResponse(const AgentResponseType requestType,
    ...
    RequestPtr requestPtr = std::make_shared<AgentActionRequest>(requestType, pluginData,
                                                                 sequenceNumber);
    RequestTakerPtr takerPtr = std::make_shared<ProtocolAgent>();
    BinaryQueue buffer;
    RequestContextPtr contextPtr = std::make_shared<RequestContext>(ResponseTakerPtr(), buffer);
    requestPtr->execute(requestPtr, takerPtr, contextPtr);
