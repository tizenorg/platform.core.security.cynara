...
static inline bool myIsNotSpace(char c, std::locale l) {
    if (c < 0)
        return true;
    return not(std::isspace(c, l));
}

// trim from start
static inline std::string &ltrim(std::string &s, std::locale &loc) {
    s.erase(s.begin(),
            std::find_if(s.begin(), s.end(), [loc](char c) { return myIsNotSpace(c, loc); }));
    return s;
}
...
static inline std::string &rtrim(std::string &s, std::locale &loc) {
    s.erase(std::find_if(s.rbegin(),
                         s.rend(),
                         [loc](char c) { return myIsNotSpace(c, loc); }).base(), s.end());
    return s;
}
                                   ...
                                   int &method)
{
    try {
        ...
        std::string key;
        std::string value;
        bool notOccurred = true; //two entries in conf file are treated as error
