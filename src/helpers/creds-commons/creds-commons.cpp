...
int cynara_creds_get_default_client_method(enum cynara_client_creds *method) {
    return Cynara::tryCatch([&] () {
        ...
        if ((ret = credsBackend.getMethodFromConfigurationFile(clientCredsMap, "client_default", methodCode))
            != CYNARA_API_SUCCESS)
            return ret;
...
int cynara_creds_get_default_user_method(enum cynara_user_creds *method) {
     return Cynara::tryCatch([&] () {
        ...
        if ((ret = credsBackend.getMethodFromConfigurationFile(userCredsMap, "user_default", methodCode))
            != CYNARA_API_SUCCESS)
            return ret;
