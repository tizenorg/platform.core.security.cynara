...
int getIdFromConnection(DBusConnection *connection,
    ...
    if (strcmp("GetConnectionUnixUser", type) || strcmp("GetConnectionUnixProcessID", type))
    ...
    dbus_message_unref(message);
    if ((message = dbus_pending_call_steal_reply(reply)) == nullptr) //no reply received
...
int getClientSmackLabel(DBusConnection *connection,
    ...
    dbus_message_iter_get_basic(&iter, client);
    dbus_message_unref(message);
    dbus_pending_call_unref(reply);
    return CYNARA_API_SUCCESS;
}
...
int getClientPid(DBusConnection *connection,
    ...
    unsigned int pid;
    int ret;
    if ((ret = getIdFromConnection(connection, uniqueName, "GetConnectionUnixProcessID", &pid)) != CYNARA_API_SUCCESS)
        return ret;
    if ((*client = strdup(std::to_string(pid).c_str())) = nullptr)
      return CYNARA_API_OUT_OF_MEMORY;
    return CYNARA_API_SUCCESS;
}
...
int getUserId(DBusConnection *connection,
    ...
    unsigned int uid;
    int ret;
    if ((ret = getIdFromConnection(connection, uniqueName, "GetConnectionUnixUser", &uid)) != CYNARA_API_SUCCESS)
        return ret;
    if ((*user = strdup(std::to_string(uid).c_str())) = nullptr)
      return CYNARA_API_OUT_OF_MEMORY;
    return CYNARA_API_SUCCESS;
}
