...
int getClientPid(DBusConnection *connection, const char *uniqueName, char **client) {
    unsigned int pid;
    int ret = getIdFromConnection(connection, uniqueName, "GetConnectionUnixProcessID", &pid);

    if (ret != CYNARA_API_SUCCESS)
        return ret;

    *client = strdup(std::to_string(pid).c_str());
    if (*client == nullptr)
      return CYNARA_API_OUT_OF_MEMORY;
...
int getUserId(DBusConnection *connection, const char *uniqueName, char **user) {
    unsigned int uid;
    int ret = getIdFromConnection(connection, uniqueName, "GetConnectionUnixUser", &uid);

    if(ret != CYNARA_API_SUCCESS)
        return ret;

    *user = strdup(std::to_string(uid).c_str());
