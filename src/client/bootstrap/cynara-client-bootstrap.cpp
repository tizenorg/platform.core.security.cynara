    ...
    const std::string& session UNUSED, const std::string& user UNUSED, const std::string& privilege)
{
    ...
    /* an assumption that SMACK label is same as pkg_id for current version of libprivilege */
    int ret = perm_app_has_permission(client.c_str(), APP_TYPE_WGT, privilege.c_str(), &is_enabled);
