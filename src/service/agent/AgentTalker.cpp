...
bool AgentTalker::sendMessage(const AgentResponseType type, const RawBuffer &data) {
    try {
        ...
        LOGE("Error in sending request to agent: <%s>", ex.what());
        return false;
    }
}

bool AgentTalker::send(const PluginData &agentData) {
    try {
        RawBuffer data(agentData.begin(), agentData.end());
        return sendMessage(CYNARA_MSG_TYPE_ACTION, data);
    } catch (const std::bad_alloc &ex) {
        LOGE("Not enough memory for building agen trequest: <%s>", ex.what());
        return false;
    }
}

bool AgentTalker::cancel(void) {
    RawBuffer data;
    return sendMessage(CYNARA_MSG_TYPE_CANCEL, data);
}
