 ...
 */
/**
 * @file        src/service/storage/InMemoryStorageBackend.cpp
...
void InMemoryStorageBackend::deleteNonIndexedFiles(void) {
    ...
    while ((direntPtr = readdir(dirPtr)) != nullptr) {
        ...
        //ignore all special files (index, guard, working dir, parent dir)
        if (0 == filename.compare(".") || 0 == filename.compare("buckets")
         || 0 == filename.compare("..") || 0 == filename.compare("guard")) {
            continue;
        }
        ...
        if (0 < filename.compare(m_backupFileNameSuffix)) {
            bucketId = filename.substr(m_bucketFileNamePrefix.length(),
                                       filename.length() - m_backupFileNameSuffix.length());
        } else {
            bucketId = filename.substr(m_bucketFileNamePrefix.length());
        }
