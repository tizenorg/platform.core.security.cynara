...
void Logic::execute(RequestContextPtr context, DescriptionListRequestPtr request UNUSED) {
    std::vector<PolicyDescription> descriptions;

    std::vector<PolicyDescription> internalPlugins = {
            PolicyDescription(PredefinedPolicyType::ALLOW, "Allow"),
            PolicyDescription(PredefinedPolicyType::DENY, "Deny")
    };

    auto externalPlugins = m_pluginManager->getPolicyDescriptions();

    descriptions.insert(descriptions.end(), internalPlugins.begin(), internalPlugins.end());
    descriptions.insert(descriptions.end(), externalPlugins.begin(), externalPlugins.end());
