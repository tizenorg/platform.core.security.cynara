...
void Logic::execute(RequestContextPtr context, AgentActionRequestPtr request) {
    ...
    if (request->type() == CYNARA_MSG_TYPE_CANCEL) {
        cancel(checkContextPtr->m_key, checkContextPtr->m_checkId, data, checkContextPtr->m_requestContext,
               checkContextPtr->m_plugin);
    } else if (request->type() == CYNARA_MSG_TYPE_ACTION) {
        update(checkContextPtr->m_key, checkContextPtr->m_checkId, data, checkContextPtr->m_requestContext,
               checkContextPtr->m_plugin);
    } else {
...
bool Logic::pluginCheck(const RequestContextPtr &context, const PolicyKey &key,
    ...
    switch (ret) {
        ...
        case ExternalPluginInterface::PluginStatus::ANSWER_NOTREADY: {
                ...
                if (!agentTalker) {
                    LOGE("Required agent talker for: <%s> could not be created.", requiredAgent.c_str());
                    result = PolicyResult(PredefinedPolicyType::DENY);
                    return true;
                }
...
bool Logic::update(const PolicyKey &key, ProtocolFrameSequenceNumber checkId,
    ...
    switch (ret) {
        ...
        case ExternalPluginInterface::PluginStatus::ANSWER_NOTREADY:
            ...
            // answerReady = check(context, key, checkId, result);
            // but for now it is useless because thi will call the same plugin, so:
            result = PolicyResult(PredefinedPolicyType::DENY);
            answerReady = true;
            break;
        default:
