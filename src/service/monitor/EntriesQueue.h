...
public:
     ...
     */
    int sizeFrom(EntryId fromEntryId);
    ...
    /*
     * Returns last valid stored entryId. Mind that last entryId can be smaller than first entryId.
     */
    int getBackEntryId(void) {
        return (m_lastBucketId * m_maxBucketSize + m_entries[m_lastBucketId].offset + 1)
                % m_maxQueueSize;
    }
