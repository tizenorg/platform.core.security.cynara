...
bool EntriesManager::isClientFilled(const ClientInfo &client) {
    return m_container.size(client.from) >= client.bufferSize;
}
...
RequestContext::ClientId EntriesManager::getFilledClientId(void) {
    for (auto registeredClient : m_registeredClients) {
        ...
        if (m_container.size(registeredClientInfo.from) >= registeredClientInfo.bufferSize) {
...
std::vector<MonitorEntry> EntriesManager::fetchEntriesForClient(RequestContext::ClientId clientId,
    ...
    int entriesToFetch = std::min(m_container.size(clientInfo.from), clientInfo.bufferSize);
...
void EntriesManager::modifyClient(RequestContext::ClientId clientId, uint16_t bufferSize) {
    auto clientIt = m_clients.find(clientId);
    if (clientIt == m_clients.end()) {
...
void EntriesManager::squashRequests() {
    ...
    if (frontId > backId) {
        ...
        auto startIt = m_entryCountMap.upper_bound(backId);
        ...
        startIt = m_entryCountMap.upper_bound(backId);
