...
TEST(dump_load, bucket) {
    ...
    const auto &epectedPolicies = bucket.policyCollection();

    ASSERT_THAT(loadedPolicies, UnorderedElementsAre(
        PolicyAtPtrEq(epectedPolicies.at(0)),
        PolicyAtPtrEq(epectedPolicies.at(1)),
        PolicyAtPtrEq(epectedPolicies.at(2))
    ));
}
