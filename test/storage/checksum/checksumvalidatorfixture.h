...
protected:
    const std::shared_ptr<std::istream> &minimalChecksumFile() {
        std::stringstream buffer;

        buffer << "_" << m_fieldSeparator << "$1$$qRPK7m23GJusamGpoGLby/" << m_recordSeparator
               << "buckets" << m_fieldSeparator << "$1$$6ZlVs5lw2nZgVmiw0BdY21" << m_recordSeparator;

        m_checksumFile = std::make_shared<std::istringstream>(buffer.str());
        return m_checksumFile;
    }

    std::shared_ptr<std::istream> m_checksumFile;
