...
TEST(ProtocolAdmin, AdminCheckResponse01) {
    auto response = std::make_shared<AdminCheckResponse>(Results::allow, VALID_BUCKET, SN::min_2);
...
TEST(ProtocolAdmin, AdminCheckResponse03) {
    auto response = std::make_shared<AdminCheckResponse>(Results::bucket_1, VALID_BUCKET, SN::min);
    auto protocol = std::make_shared<ProtocolAdmin>();
    testResponse(response, protocol);
}

TEST(ProtocolAdmin, AdminCheckResponse04) {
    auto response = std::make_shared<AdminCheckResponse>(Results::none, NO_BUCKET, SN::max_2);
    auto protocol = std::make_shared<ProtocolAdmin>();
    testResponse(response, protocol);
}

TEST(ProtocolAdmin, AdminCheckResponse05) {
    auto response = std::make_shared<AdminCheckResponse>(Results::plugin_1, VALID_BUCKET, SN::max);
    auto protocol = std::make_shared<ProtocolAdmin>();
    testResponse(response, protocol);
}

TEST(ProtocolAdmin, AdminCheckResponse06) {
    auto response = std::make_shared<AdminCheckResponse>(Results::plugin_2, NO_BUCKET, SN::max_1);
    auto protocol = std::make_shared<ProtocolAdmin>();
    testResponse(response, protocol);
}

TEST(ProtocolAdmin, AdminCheckResponse07) {
    auto response = std::make_shared<AdminCheckResponse>(Results::plugin_3, VALID_BUCKET, SN::mid);
    auto protocol = std::make_shared<ProtocolAdmin>();
    testResponse(response, protocol);
}
