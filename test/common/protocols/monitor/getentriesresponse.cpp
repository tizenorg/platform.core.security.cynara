...
const int64_t TV_SEC_MAX = std::numeric_limits<int64_t>::max();
const int64_t TV_SEC_HALF = (TV_SEC_MAX - TV_SEC_MIN) / 2;
const int64_t TV_NSEC_MIN = 0;
const int64_t TV_NSEC_1 = 1;
const int64_t TV_NSEC_MAX = std::numeric_limits<int64_t>::max();
const int64_t TV_NSEC_HALF = (TV_SEC_MAX - TV_SEC_MIN) / 2;

template<>
void compare(const Cynara::MonitorEntry &m1, const Cynara::MonitorEntry &m2) {
    EXPECT_EQ(m1.key(), m2.key());
    EXPECT_EQ(m1.result(), m2.result());
    EXPECT_EQ(m1.timestamp(), m2.timestamp());
}
