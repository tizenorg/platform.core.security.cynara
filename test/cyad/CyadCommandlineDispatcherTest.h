...
public:
    void SetUp(void) {
        ...
        m_policyDescs = static_cast<DescrType **>(realloc(m_policyDescs, arrSize));
...
protected:
    ...
    void setupDescriptionsExpectation() {
        ...
        EXPECT_CALL(m_adminApi, cynara_admin_list_policies_descriptions(_, NotNull()))
            .WillOnce(DoAll(SetArgPointee<1>(m_policyDescs), Return(CYNARA_API_SUCCESS)));
    }
...
private:
    cynara_admin_policy_descr **m_policyDescs = nullptr;
