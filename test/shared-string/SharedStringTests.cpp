...
TEST(SharedString, ConstructorDestructor) {
    ...
    ASSERT_EQ(getRefCount(pattern), StringStorageEx::NOT_FOUND);
    {
        SharedString ex(pattern);
        ASSERT_EQ(getRefCount(pattern), 1);
        SharedString ex2(pattern);
        ASSERT_EQ(getRefCount(pattern), 2);
    }
    ASSERT_EQ(getRefCount(pattern), StringStorageEx::NOT_FOUND);
}
...
TEST(SharedString, CopyOperator) {
    ...
    ASSERT_EQ(getRefCount(pattern), StringStorageEx::NOT_FOUND);
    {
        ...
        ASSERT_EQ(getRefCount(pattern), 1);
        {
            ...
            ASSERT_EQ(getRefCount(pattern2), 1);
            copy = ex;
            ASSERT_EQ(getRefCount(pattern2), StringStorageEx::NOT_FOUND);
            ASSERT_EQ(getRefCount(pattern), 2);
        }
        ASSERT_EQ(getRefCount(pattern), 1);
    }
